...
const HABIT_OPTIONS = [
  ...
  {
    habit_id: "reusable",
    title: "Reusable Container / Cup",
    category: "Waste",
    co2_saved_kg: 0.8,
    icon: "♻️",
  },
  {
    habit_id: "energy_save",
    title: "Cold Wash / Energy Saver",
    category: "Energy",
    co2_saved_kg: 0.6,
    icon: "🧺",
  },
...
export default function DashboardPage() {
  ...
  // Load user session and fetch historical habit logs from Supabase
  useEffect(() => {
  ...
  // Handle logging a habit securely to Supabase
  ...
  // Calculate live dynamic metrics from database logs
  const totalCO2Saved = userLogs.reduce(
    (acc, log) => acc + Number(log.co2_saved_kg || 0),
    0
  );
  ...
  return (
    <div className="min-h-screen bg-emerald-950/5 text-foreground flex flex-col justify-between relative overflow-hidden">
      ...
      <header className="px-6 py-4 border-b border-emerald-900/10 backdrop-blur-md bg-background/80 flex justify-between items-center max-w-6xl mx-auto w-full z-10">
        <Link
          href="/"
          className="font-bold text-xl tracking-tight text-[#0f382c] flex items-center gap-2"
        >
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="text-xs font-semibold px-3.5 py-1.5 border border-emerald-900/20 rounded-full hover:bg-emerald-100/50 transition"
          >
            Sign Out
          </button>
        </div>
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full px-6 py-10 z-10 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-5 border border-emerald-900/10 rounded-2xl bg-card/80 backdrop-blur shadow-sm">
            ...
            <div className="text-3xl font-extrabold text-[#0f382c]">
              {totalCO2Saved.toFixed(1)} <span className="text-sm font-normal text-muted-foreground">kg</span>
            </div>
            <span className="text-xs text-muted-foreground mt-1 block">
              Cumulative database-verified impact
            </span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 border border-emerald-900/10 rounded-2xl bg-card/80 backdrop-blur p-6 shadow-sm">
            <h2 className="text-lg font-bold text-foreground mb-4">
              Log Sustainable Choice
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {HABIT_OPTIONS.map((option) => (
                <div
                  key={option.habit_id}
                  className="p-4 rounded-xl border border-emerald-900/10 bg-background/60 flex items-center justify-between gap-3"
                >
          ...
          <div className="border border-emerald-900/10 rounded-2xl bg-card/80 backdrop-blur p-6 shadow-sm">
            ...
            {userLogs.length === 0 ? (
              <p className="text-xs text-muted-foreground">
                No logged habits yet. Select an action to start building your verified impact profile.
              </p>
            ) : (
              <div className="space-y-3 max-h-[320px] overflow-y-auto pr-1">
                {userLogs.map((log) => (
                  <div
                    key={log.id}
                    className="p-3 rounded-xl border border-emerald-900/10 bg-background/50 flex justify-between items-center text-xs"
                  >
                    <div>
